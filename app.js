(() => {
  ...
  async function init(){
    ...
    $("#student-login-form").onsubmit=studentLogin;$("#admin-login-form").onsubmit=adminLogin;$$('.logout-btn').forEach(b=>b.onclick=logout);$$('.refresh-btn').forEach(b=>b.onclick=()=>state.view&&navigate(state.view));
  ...
  async function demoPreview(){try{const previewKey=$("#demo-student-select").value,r=await api("studentLogin",{previewKey});state.mode="student";state.auth={previewKey};state.student=r.student;enterApp("student-home");}catch(e){toast(e.message);}}
  async function studentLogin(e){e.preventDefault();try{const studentId=$("#student-id").value.trim(),loginPin=$("#student-login-pin").value.trim(),r=await api("studentLogin",{studentId,loginPin});state.mode="student";state.auth={studentId,loginPin};state.student=r.student;enterApp("student-home");}catch(err){toast(err.message);}}
  async function adminLogin(e){e.preventDefault();try{const pin=$("#admin-pin").value.trim();await api("adminLogin",{pin});state.mode="admin";state.auth={pin};enterApp("admin-dashboard");}catch(err){toast(err.message);}}
  function enterApp(view){$("#login-screen").classList.add("hidden");$("#app-shell").classList.remove("hidden");$("#admin-sidebar").classList.toggle("hidden",state.mode!=="admin");$("#student-bottom-nav").classList.toggle("hidden",state.mode!=="student");$(".main").classList.toggle("student-mode",state.mode==="student");navigate(view);}
  ...
  async function navigate(view){state.view=view;$$('#admin-nav [data-view],#student-bottom-nav [data-view]').forEach(b=>b.classList.toggle('active',b.dataset.view===view));const titles={
    ...
    "student-home":["TODAY","홈"],"student-schedule":["MY CALENDAR","내 달력"],"student-substitute":["SUBSTITUTE","대타 모집"],"student-extra":["EXTRA SHIFT","추가근무"],"student-records":["HISTORY","내 기록"]};$("#page-eyebrow").textContent=titles[view]?.[0]||"WORKSPACE";$("#page-title").textContent=titles[view]?.[1]||"";$("#page-content").innerHTML='<div class="card"><div class="empty">불러오는 중...</div></div>';
    try{if(state.mode==="admin")state.adminData=await api("getAdminDashboard",state.auth);else state.studentData=await api("getStudentDashboard",state.auth);render(view);}catch(e){$("#page-content").innerHTML=`<div class="card"><div class="empty">${esc(e.message)}</div></div>`;}}
  ...
  async function actAndReload(action,params,msg,view){try{await api(action,{...state.auth,...params});toast(msg);navigate(view);}catch(e){toast(e.message);}}
